...
class RelPosEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def forward(self, src, src_mask=None, is_causal=True, src_key_padding_mask=None):
        ...
        combined_mask = combined_mask.reshape(B*H, T, T)
...
class GRUDecoder(nn.Module):
    def __init__(
        ...
        # --- Transformer Encoder replacing GRU ---
        self.input_proj = nn.Linear(self.neural_dim * self.kernelLen, self.hidden_dim)
    ...
    def forward(self, neuralInput, dayIdx):     # neuralInput.shape (batchsize=128, seqlen=859, feature=256)
        ...
        # Transformer
        x = stridedInputs.permute(1, 0, 2)   # (B,T,D) -> (T,B,D)
        
        x = self.input_proj(x)
        
        hid = self.transformer_encoder(x)
        hid = hid.permute(1, 0, 2)   # (T,B,D) -> (B,T,D)
